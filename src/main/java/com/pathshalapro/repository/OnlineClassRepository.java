...
public interface OnlineClassRepository extends JpaRepository<OnlineClass, Long> {
    ...
    @Query("SELECT oc FROM OnlineClass oc WHERE oc.school.id = :schoolId AND oc.scheduledAt BETWEEN :start AND :end " +
           "AND oc.isDeleted = false ORDER BY oc.scheduledAt")
